...
4. **Template `/print/orders.html`**: ~~mismatch nei nomi campo API~~ **RISOLTO** — corretti `IDOrder`, `OrderNumber`, `OrderQuantity` nel template.
5. **Template `/print/generic.html`**: ~~chiave `script` mancante in italiano~~ **RISOLTO** — chiave aggiunta a `PRINT_UI`. Rimane il titolo del pannello settings hardcoded `<h2>Settings</h2>`.
5bis. **Multi-select ordini su `/print/generic`**: implementato come da sezione 9.2 (campo 6). Le ricerche usano `/print/api/orders/search`; nel payload di stampa viene inviato `order_ids` con gli ordini selezionati. Tutti gli ordini selezionati vengono tracciati in `ind.LabelPrintLogOrders` (tabella creata da `sql/add_label_print_log_orders.sql`); la colonna `LabelPrintLog.OrderId` continua a registrare solo il primo ordine, per retrocompatibilità. Se la tabella associazioni non è ancora stata creata sul DB, la stampa funziona comunque e le associazioni vengono solo saltate (warning nel log del server).
5ter. **Ricerca per prodotto e import associazioni Excel**: in `/print/orders`, se il testo cercato coincide con codici prodotto, la API `/print/api/orders/search` restituisce **tutti** gli ordini aperti (`Orders.IsFinished = 0`) di quei prodotti, senza limite di 50 righe; altrimenti rimane la ricerca generica TOP 50. Aggiunti endpoint `GET /print/api/labels/template` (download template Excel) e `POST /print/api/labels/import` (upload Excel): colonne `CodiceProdotto, CodiceEtichetta, QuantitaPerPezzo`; **sostituisce** le associazioni attive in `ind.BomIndirectMaterials` dei prodotti nel file (le etichette devono essere nella famiglia `Labels`; codici non risolti vengono saltati con warning). Le associazioni sono quelle già usate da `/print/api/orders/labels` e sono disponibili anche per la stampa generica.
