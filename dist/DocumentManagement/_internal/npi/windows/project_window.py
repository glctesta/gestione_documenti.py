...
from .project_hierarchy_window import ProjectHierarchyWindow
...
class ProjectWindow(tk.Toplevel):
    ...
    def __init__(self, master, npi_manager, lang, project_id, master_app, logged_in_user: str,
        ...
        self.sort_column = None  # Colonna corrente per ordinamento (None = ordinamento di default)
        self.sort_reverse = False  # Direzione ordinamento (False = ascendente, True = discendente)
    ...
    def _create_widgets(self):
        ...
        # Bottone per resettare l'ordinamento
        ttk.Button(toolbar_row2, text=self.lang.get('reset_sort', '🔄 Reset Ordinamento'), command=self._reset_sort).pack(side=tk.LEFT, padx=(10, 5))
        
        # Bottone per configurare l'ordinamento di default
        ttk.Button(toolbar_row2, text=self.lang.get('config_sort', '⚙️ Configura Ordinamento'), command=self._open_sort_config).pack(side=tk.LEFT, padx=5)
        ...
        # Info gerarchia
        hierarchy_info = ttk.Frame(hierarchy_frame)
        hierarchy_info.pack(fill=tk.X, padx=5, pady=5)
        ...
        self.children_count_label.pack(side=tk.LEFT, padx=10)
        ...
        self.tree.heading('Category', text=self.lang.get('col_category', 'Categoria'), command=lambda: self._on_column_click('Category'))
        self.tree.heading('Name', text=self.lang.get('col_task', 'Task'), command=lambda: self._on_column_click('Name'))
        self.tree.heading('Owner', text=self.lang.get('col_owner', 'Assegnato a'), command=lambda: self._on_column_click('Owner'))
        self.tree.heading('Status', text=self.lang.get('col_status', 'Stato'), command=lambda: self._on_column_click('Status'))
        self.tree.heading('StartDate', text=self.lang.get('col_start_date', 'Inizio'), command=lambda: self._on_column_click('StartDate'))
        self.tree.heading('DueDate', text=self.lang.get('col_due_date', 'Scadenza'), command=lambda: self._on_column_click('DueDate'))
    ...
    def _load_data_and_populate_ui(self):
        try:
            ...
            if not self.progetto.OwnerID:
                ...
                try:
                    ...
                    config_window = NpiConfigWindow(
                        ...
                        db=getattr(self.master_app, 'db', None),  # 🆕 Passa db per HR lookup
            ...
            # IMPORTANTE: il confronto avviene per SoggettoId (intero), NON per nome,
            # perché il nome in vw_Soggetti (NomeSoggetto) può differire dal nome
            # restituito dal sistema HR (auth_result.EmployeeName) usato come logged_in_user.
            project_owner_id = self.progetto.OwnerID if self.progetto else None

            # Recupera il SoggettoId dell'utente loggato cercandolo per nome nella mappa.
            # Prima prova corrispondenza esatta, poi case-insensitive come fallback.
            logged_user_id = self.soggetti_map.get(self.logged_in_user)
            if logged_user_id is None and self.logged_in_user:
                logged_in_lower = self.logged_in_user.lower()
                for nome, sid in self.soggetti_map.items():
                    if nome and nome.lower() == logged_in_lower:
                        logged_user_id = sid
                        logger.info(f"🔍 Corrispondenza case-insensitive: '{nome}' → ID={sid}")
                        break
                if logged_user_id is None:
                    logger.warning(
                        f"⚠️ '{self.logged_in_user}' non trovato in soggetti_map. "
                        f"Nomi disponibili (primi 10): {list(self.soggetti_map.keys())[:10]}"
                    )
            logger.info(
                f"🔍 Owner check: project_owner_id={project_owner_id}, "
                f"logged_in_user='{self.logged_in_user}', logged_user_id={logged_user_id}"
            )

            if project_owner_id is None:
                # Nessun owner assegnato al progetto → accesso completo
                self.is_project_owner = True
                logger.info("Nessun owner assegnato al progetto: accesso completo garantito.")
            elif logged_user_id and project_owner_id == logged_user_id:
                # Confronto per ID: l'utente loggato È l'owner del progetto
                self.is_project_owner = True
                logger.info(f"✅ '{self.logged_in_user}' (ID={logged_user_id}) è l'owner del progetto.")
            else:
                # Fallback: confronto per nome (nel caso il SoggettoId non sia trovato nella mappa)
                project_owner_name = self.soggetti_map_rev.get(project_owner_id)
                self.is_project_owner = bool(
                    project_owner_name and project_owner_name == self.logged_in_user
                )
                logger.info(
                    f"🔍 Confronto per nome: project_owner_name='{project_owner_name}' "
                    f"vs logged_in_user='{self.logged_in_user}' → is_owner={self.is_project_owner}"
                )
    ...
    def _populate_treeview(self):
        ...
        if self.sort_column is None:
            # Ordinamento di default configurato dall'utente
            sorted_tasks = self._apply_default_sort(filtered_tasks)
        else:
            # Ordinamento manuale per colonna selezionata
            sorted_tasks = self._sort_tasks_by_column(filtered_tasks, self.sort_column, self.sort_reverse)
    ...
    def _on_column_click(self, column):
        """Gestisce il click su un'intestazione di colonna per ordinare."""
        # Se la stessa colonna è cliccata, inverti la direzione
        if self.sort_column == column:
            self.sort_reverse = not self.sort_reverse
        else:
            # Nuova colonna selezionata, ordina in modo ascendente
            self.sort_column = column
            self.sort_reverse = False
        
        # Aggiorna le intestazioni con gli indicatori di direzione
        self._update_column_headers()
        
        # Riordina il treeview
        self._populate_treeview()
    
    def _update_column_headers(self):
        """Aggiorna le intestazioni delle colonne con gli indicatori di direzione."""
        columns_config = {
        ...
        for col, base_text in columns_config.items():
    ...
    def _sort_tasks_by_column(self, tasks, column, reverse):
        """Ordina i task in base alla colonna selezionata."""
        from datetime import datetime
        
        def get_sort_key(task):
            """Restituisce la chiave di ordinamento per un task in base alla colonna."""
            if column == 'Category':
                # Ordina per nome categoria
                cat = task.task_catalogo.categoria.Category if task.task_catalogo and task.task_catalogo.categoria else ""
                return (cat.lower() if cat else "zzz", task.TaskProdottoID)  # zzz per mettere vuoti alla fine
            
            elif column == 'Name':
                # Ordina per nome task
                name = task.task_catalogo.NomeTask if task.task_catalogo else ""
                return (name.lower() if name else "zzz", task.TaskProdottoID)
            
            elif column == 'Owner':
                # Ordina per nome proprietario
                owner = task.owner.Nome if task.owner else ""
                return (owner.lower() if owner else "zzz", task.TaskProdottoID)
            
            elif column == 'Status':
                # Ordina per stato con ordine logico
                status_order = {'Da Fare': 1, 'In Lavorazione': 2, 'Completato': 3, 'Bloccato': 4}
                status = task.Stato if task.Stato else 'Da Fare'
                return (status_order.get(status, 5), task.TaskProdottoID)
            
            elif column == 'StartDate':
                # Ordina per data di inizio
                date = task.DataInizio if task.DataInizio else datetime(2099, 12, 31)
                return (date, task.TaskProdottoID)
            
            elif column == 'DueDate':
                # Ordina per data di scadenza
                date = task.DataScadenza if task.DataScadenza else datetime(2099, 12, 31)
                return (date, task.TaskProdottoID)
            
            else:
                return (0, task.TaskProdottoID)
        
        return sorted(tasks, key=get_sort_key, reverse=reverse)
    
    def _reset_sort(self):
        """Resetta l'ordinamento al default (topologico)."""
    ...
    def _load_sort_config(self):
        """Carica la configurazione di ordinamento dal file JSON locale."""
        import json
        import os
        
        config_file = os.path.join(os.path.expanduser('~'), '.npi_sort_config.json')
        
        try:
            if os.path.exists(config_file):
                with open(config_file, 'r') as f:
                    self.default_sort_config = json.load(f)
                logger.info(f"Configurazione ordinamento caricata: {self.default_sort_config}")
            else:
                # Default: Task, DataScadenza
                self.default_sort_config = [('Name', False), ('DueDate', False)]
                logger.info("Configurazione ordinamento default applicata")
        except Exception as e:
            logger.error(f"Errore caricamento configurazione ordinamento: {e}")
            self.default_sort_config = [('Name', False), ('DueDate', False)]
    
    def _save_sort_config(self):
        """Salva la configurazione di ordinamento nel file JSON locale."""
        import json
        import os
        ...
        try:
            with open(config_file, 'w') as f:
                json.dump(self.default_sort_config, f)
            logger.info(f"Configurazione ordinamento salvata: {self.default_sort_config}")
        except Exception as e:
            logger.error(f"Errore salvataggio configurazione ordinamento: {e}")
    
    def _apply_default_sort(self, tasks):
        """Applica l'ordinamento di default multi-campo configurato."""
        if not self.default_sort_config or not tasks:
            return tasks
        
        # Ordina per ogni campo in ordine inverso di priorità
        # (l'ultimo campo ha priorità minore, il primo ha priorità massima)
        sorted_tasks = tasks[:]
        for column, reverse in reversed(self.default_sort_config):
            sorted_tasks = self._sort_tasks_by_column(sorted_tasks, column, reverse)
        
        return sorted_tasks
    
    def _open_sort_config(self):
        """Apre la finestra di configurazione ordinamento."""
        from .sort_config_window import SortConfigWindow
        
        def on_save(new_config):
            """Callback chiamato quando l'utente salva la configurazione."""
            self.default_sort_config = new_config
            self._save_sort_config()
            self._reset_sort()  # Applica il nuovo ordinamento di default
            messagebox.showinfo(
                self.lang.get('success_title', 'Successo'),
                self.lang.get('sort_config_saved', 'Configurazione ordinamento salvata con successo.'),
                parent=self
            )
        
        SortConfigWindow(self, self.lang, self.default_sort_config, on_save)
    ...
    def _save_task_details(self):
        ...
        try:
            ...
            # Ripristina il filtro categoria e l'ordinamento dopo il ricaricamento
            if current_category_filter:
                self.category_filter_var.set(current_category_filter)
            
            # Ripristina lo stato di ordinamento
            self.sort_column = current_sort_column
    ...
    def _delete_task_prodotto(self):
        ...
        try:
            ...
            try:
                ...
                stmt = select(TaskDependency).where(
                    TaskDependency.DependsOnTaskProdottoID == self.current_task_id
                )
                ...
                if dependent_tasks:
                    ...
                    for dep in dependent_tasks:
                        dep_task = session.get(TaskProdotto, dep.TaskProdottoID)
        ...
        try:
            # Elimina il task dal progetto usando SQL diretto per evitare problemi di cascade
            session = self.npi_manager._get_session()
            try:
                # Usa DELETE SQL diretto invece di session.delete() per evitare il caricamento delle relazioni
                from sqlalchemy import text
                result = session.execute(
                    text("DELETE FROM [Traceability_RS].[dbo].[TaskProdotto] WHERE TaskProdottoID = :task_id"),
    ...
    def _open_hierarchy_window(self):
        """Apre la finestra dedicata per gestire la gerarchia progetti."""
        try:
            if not self.progetto:
                messagebox.showwarning(
                    "Attenzione",
                    "Carica prima i dati del progetto.",
                    parent=self
                )
                return
            
            # Ottieni il cliente del progetto
            cliente = self.progetto.prodotto.Cliente if self.progetto.prodotto else None
            
            if not cliente:
                messagebox.showwarning(
                    "Attenzione",
                    "Il progetto non ha un cliente associato. Impossibile gestire la gerarchia.",
                    parent=self
                )
                return
            
            # Apri la finestra di gestione gerarchia
            ProjectHierarchyWindow(
                parent=self,
                npi_manager=self.npi_manager,
                lang=self.lang,
                parent_project_id=self.project_id,
                parent_project_name=self.progetto.prodotto.NomeProdotto if (not self.progetto.NomeProgetto and self.progetto.prodotto and self.progetto.prodotto.NomeProdotto) else (self.progetto.prodotto.CodiceProdotto if (not self.progetto.NomeProgetto and self.progetto.prodotto and self.progetto.prodotto.CodiceProdotto) else (self.progetto.NomeProgetto or f"Progetto {self.project_id}")),
                parent_client=cliente
            )
            
            # Aggiorna il conteggio figli dopo la chiusura della finestra
    ...
    def _populate_project_hierarchy(self):
        ...
        try:
            # 🔄 NOTA: Il vecchio codice per popolare il combo padre è stato rimosso
            # perché ora usiamo una finestra dedicata (ProjectHierarchyWindow)
            # per gestire la gerarchia. Il progetto aperto è sempre il PADRE.
            
            # Aggiorna solo il conteggio figli
            self._update_children_count()
