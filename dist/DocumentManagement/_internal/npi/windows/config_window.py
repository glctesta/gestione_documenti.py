...
class SubjectManagementFrame(ttk.Frame):
    ...
    def __init__(self, master, npi_manager, lang, db=None, **kwargs):
        super().__init__(master, **kwargs)
        self.npi_manager = npi_manager
        self.lang = lang
        self.db = db  # 🆕 Connessione DB principale per HR lookup
        ...
        # 🆕 Label indicatore HR status (Interno / Esterno)
        self.hr_status_var = tk.StringVar(value="")
        self.hr_status_label = ttk.Label(form_frame, textvariable=self.hr_status_var, foreground='gray')
        self.hr_status_label.grid(row=len(labels), column=1, sticky=tk.W, padx=5)

        button_frame = ttk.Frame(form_frame)
        button_frame.grid(row=len(labels) + 1, column=0, columnspan=2, pady=20, sticky=tk.E)
    ...
    def _clear_form(self, clear_selection=True):
        ...
        self.hr_status_label.config(foreground='gray')
    ...
    def _save_subject(self):
        ...
        # 🆕 HR LOOKUP: cerca il nome nel sistema HR prima di salvare
        is_external = False
        nome_da_salvare = nome

        if self.db is not None:
            try:
                hr_name = self.npi_manager.lookup_employee_by_name(self.db, nome)
                if hr_name:
                    # Trovato in HR: usa il nome canonico
                    nome_da_salvare = hr_name
                    is_external = False
                    self.hr_status_var.set(f"✅ Interno HR: {hr_name}")
                    self.hr_status_label.config(foreground='green')
                    # Aggiorna il campo visivo con il nome canonico
                    self.fields['Nome'].delete(0, tk.END)
                    self.fields['Nome'].insert(0, hr_name)
                else:
                    # Non trovato in HR: salva come esterno
                    nome_da_salvare = nome
                    is_external = True
                    self.hr_status_var.set("⚠️ Esterno (non trovato in HR)")
                    self.hr_status_label.config(foreground='orange')
                    messagebox.showinfo(
                        self.lang.get('info_title', 'Informazione'),
                        self.lang.get(
                            'msg_subject_not_in_hr',
                            f'"{nome}" non trovato nel sistema HR.\n\n'
                            'Il soggetto verrà salvato come ESTERNO.\n'
                ...
                import logging
                logging.getLogger(__name__).error(f"Errore HR lookup: {e}", exc_info=True)
                # In caso di errore nel lookup, salva comunque senza bloccare
                nome_da_salvare = nome
                is_external = False
        else:
            # Nessuna connessione DB disponibile: salva senza lookup
            nome_da_salvare = nome
            is_external = False
...
class ProductManagementFrame(ttk.Frame):
    def __init__(self, master, npi_manager, lang, **kwargs):
        ...
        # 🆕 Pulsante soft-delete progetto
        self.delete_project_button = ttk.Button(
            project_buttons_frame,
            text=self.lang.get('btn_delete_npi_project', '🗑 Elimina Progetto'),
            command=self._delete_npi_project,
            state=tk.DISABLED
        )
    ...
    def _create_npi_project(self):
        ...
        try:
            ...
            # 🆕 Pre-check: se add_defaults=True ma non ci sono default configurati, avvisa l'utente
            if add_defaults and not self.npi_manager.has_default_categories_and_tasks():
                risposta = messagebox.askyesno(
                    self.lang.get('warning_title', 'Attenzione'),
                    self.lang.get(
                        'msg_no_default_tasks_precheck',
                        '⚠️ Nessuna categoria/task di default configurata.\n\n'
                        'Il progetto verrà creato SENZA task.\n\n'
                        'Per aggiungere task di default, vai nella scheda "Default Catalogo"\n'
                        'e marca le categorie/task desiderati.\n\n'
                        'Vuoi procedere comunque con la creazione del progetto vuoto?'
                    ),
                    parent=self
                )
                if not risposta:
                    return
                add_defaults = False  # Crea il progetto vuoto senza tentare di copiare i default
    ...
    def _delete_npi_project(self):
        """Soft-delete del progetto NPI: imposta DateOut=GETDATE() se non ha dati collegati."""
        if self.selected_product_id is None:
            messagebox.showwarning(
                self.lang.get('warning_title', 'Attenzione'),
                self.lang.get('warning_select_product', 'Seleziona un prodotto dalla lista.'),
                parent=self
            )
            return
        ...
        conferma = messagebox.askyesno(
            ...
            self.lang.get(
                ...
                f'⚠️ Sei sicuro di voler cancellare il progetto NPI?\n\n'
                f'Il progetto verrà marcato come eliminato e non sarà più visibile.\n'
                f'Questa operazione è consentita solo se il progetto non ha task con dati registrati.'
            ),
            parent=self
        )
        if not conferma:
        ...
        ok, errore = self.npi_manager.soft_delete_progetto(progetto.ProgettoId)
        if ok:
            messagebox.showinfo(
                self.lang.get('success_title', 'Successo'),
                self.lang.get('msg_project_deleted', 'Progetto eliminato con successo.'),
                parent=self
            )
            self._clear_form()
            self._load_products()
