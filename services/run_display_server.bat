@echo off
REM Avvia il display server Kanban (porta 6505) in background, senza console.
REM Alimenta i monitor di reparto WIP/REPAIR (dashboard + modalita' 3D).
...
if exist ".venv\Scripts\pythonw.exe" (
    ".venv\Scripts\pythonw.exe" wip_kanban\display_server.py
) else (
    pythonw.exe wip_kanban\display_server.py
)
