...
class GestoreNPI:
    ...
    def _ensure_default_flags_schema(self):
        """Assicura che le colonne DefaultCategory, DefaultTask, External e DateOut esistano."""
        try:
            with self.engine.begin() as conn:
                ...
                END
                """))
                # Colonna External per distinguere soggetti interni (HR) da esterni
                conn.execute(text("""
                IF COL_LENGTH('dbo.Soggetti', 'External') IS NULL
                BEGIN
                    ALTER TABLE dbo.Soggetti
    ...
    def lookup_employee_by_name(self, db, search_name: str):
        """
        Cerca un dipendente nel sistema HR tramite corrispondenza parziale del nome.

        Usa la connessione pyodbc principale (db.cursor) per eseguire una query
        cross-database sui sistemi HR (resetservices + employee).

        Args:
            db: oggetto Database principale con attributo `cursor` (pyodbc)
            search_name: nome (parziale o completo) da cercare

        Returns:
            str: nome canonico HR in formato UPPER('EmployeeName EmployeeSurname'),
                 oppure None se non trovato o in caso di errore.
        """
        ...
        query = """
            ...
            INNER JOIN employee.dbo.EmployeeHireHistory AS h
                ...
                AND h.EmployeerId = 2
            WHERE UPPER(e.EmployeeName + ' ' + e.EmployeeSurname) LIKE UPPER(?)
        """
        try:
            pattern = f"%{search_name.strip()}%"
            db.cursor.execute(query, pattern)
            row = db.cursor.fetchone()
            if row and row[0] and row[0] != '#ND':
                logger.info(f"✅ HR lookup '{search_name}' → '{row[0]}'")
                return row[0]
            else:
                logger.info(f"❌ HR lookup '{search_name}' → nessun risultato")
                return None
        except Exception as e:
            logger.error(f"Errore in lookup_employee_by_name per '{search_name}': {e}", exc_info=True)
            return None

    def create_soggetto(self, data):
        """
        Crea un nuovo soggetto scrivendo direttamente su dbo.Soggetti (non sulla view).
        Questo garantisce che il campo External venga scritto correttamente.
        """
        try:
            with self.engine.begin() as conn:
                result = conn.execute(
                    text("""
                        INSERT INTO dbo.Soggetti
                            (NomeSoggetto, Tipo, Email, MSTeamsUserID, [External])
                        OUTPUT INSERTED.SoggettoId
                        VALUES
                            (:nome, :tipo, :email, :teams, :external)
                    """),
                    {
                        'nome': data.get('Nome', ''),
                        'tipo': data.get('Tipo', 'Interno'),
                        'email': data.get('Email', ''),
                        'teams': data.get('TeamsAddress', ''),
                        'external': 1 if data.get('External', False) else 0,
                    }
                )
                new_id = result.scalar()
                logger.info(f"Soggetto creato: ID={new_id}, Nome='{data.get('Nome')}', External={data.get('External', False)}")
                return new_id
        except Exception as e:
            logger.error(f"Errore in create_soggetto: {e}", exc_info=True)
            raise

    def update_soggetto(self, soggetto_id, data):
        """
        Aggiorna un soggetto esistente scrivendo direttamente su dbo.Soggetti (non sulla view).
        Questo garantisce che il campo External venga aggiornato correttamente.
        """
        try:
            with self.engine.begin() as conn:
                conn.execute(
                ...
                logger.info(f"Soggetto aggiornato: ID={soggetto_id}, Nome='{data.get('Nome')}', External={data.get('External', False)}")
                return soggetto_id
        except Exception as e:
            logger.error(f"Errore in update_soggetto: {e}", exc_info=True)
            raise
    ...
    def create_progetto_npi_for_prodotto(self, prodotto_id, version=None, owner_id=None, descrizione=None, add_defaults: bool = False):
        ...
        try:
            ...
            if add_defaults:
                ...
                logger.info(
                    f"create_progetto_npi_for_prodotto: add_defaults=True, "
                    f"trovati {len(tasks_catalogo)} task default da copiare nel progetto."
                )
            else:
                tasks_catalogo = session.scalars(select(TaskCatalogo)).all()
                logger.info(
                    f"create_progetto_npi_for_prodotto: add_defaults=False, "
                    f"copio tutti i {len(tasks_catalogo)} task del catalogo."
                )
            ...
            logger.info(
                f"create_progetto_npi_for_prodotto: aggiunti {len(tasks_catalogo)} TaskProdotto "
                f"alla wave {nuova_wave.WaveID}."
            )
    ...
    def soft_delete_progetto(self, progetto_id: int) -> tuple:
        """
        Soft-delete di un progetto NPI: imposta DateOut=GETDATE().
        Restituisce (True, '') se ok, (False, messaggio_errore) altrimenti.
        La cancellazione è bloccata se il progetto ha task con dati registrati.
        """
        session = self._get_session()
        try:
            # Verifica che il progetto esista e non sia già cancellato
            progetto = session.get(ProgettoNPI, progetto_id)
            if not progetto:
                return False, f"Progetto {progetto_id} non trovato."
            if progetto.DateOut is not None:
                return False, "Il progetto è già stato cancellato."

            # Verifica che non ci siano TaskProdotto con dati (stato non NULL)
            task_count = session.scalar(
                select(func.count(TaskProdotto.TaskProdottoID))
            ...
            if task_count > 0:
                return False, (
                    f"Impossibile cancellare: il progetto ha {task_count} task con dati registrati.\n"
                    "Rimuovere prima i dati dai task o completare il progetto."
                )

            # Soft-delete: imposta DateOut con GETDATE()
            session.execute(
                text("UPDATE dbo.ProgettiNPI SET DateOut = GETDATE() WHERE ProgettoID = :pid"),
                {"pid": progetto_id}
            )
            session.commit()
            logger.info(f"Progetto {progetto_id} soft-deleted (DateOut impostato).")
            return True, ""
        except Exception as e:
            session.rollback()
            logger.error(f"Errore soft_delete_progetto {progetto_id}: {e}")
            return False, str(e)
        finally:
            session.close()
...
<div class="wrap">
  ...
  <div class="ftr">
    ...
    def get_all_overdue_tasks(self):
        """
        Recupera TUTTI i task scaduti di progetti attivi (non chiusi, non cancellati).
        
        Returns:
            Lista di dict con: project_name, customer, product_code, category,
            task_name, owner_name, owner_email, due_date, days_late, status
        """
        session = self._get_session()
        try:
            today = datetime.now().date()
            ...
            for row in tasks:
                ...
                days_late = (today - dd).days if dd else 0
            ...
            logger.info(f"get_all_overdue_tasks: trovati {len(result)} task scaduti")
            return result

        except Exception as e:
            logger.error(f"Errore in get_all_overdue_tasks: {e}", exc_info=True)
            return []
        finally:
            session.close()
    ...
    def export_overdue_tasks_to_excel(self, tasks_data=None):
        ...
        Args:
            tasks_data: Lista di dict (da get_all_overdue_tasks). Se None, li recupera.
        ...
        try:
            ...
            raise ImportError("La libreria 'openpyxl' è necessaria. pip install openpyxl")

        if tasks_data is None:
            tasks_data = self.get_all_overdue_tasks()
        if not tasks_data:
            raise ValueError("Nessun task scaduto da esportare.")
        ...
        ws[f'A{row}'] = "NPI Overdue Tasks Report"
        ws[f'A{row}'].font = title_font
        ws[f'A{row}'].alignment = Alignment(horizontal='center')
        row += 1

        ws.merge_cells(f'A{row}:I{row}')
        ws[f'A{row}'] = f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} — Total overdue: {len(tasks_data)}"
        ws[f'A{row}'].font = subtitle_font
        ws[f'A{row}'].alignment = Alignment(horizontal='center')
        row += 2

        headers = ["NPI Project", "Customer", "Product", "Family", "Task",
                    "Owner", "Due Date", "Days Late", "Status"]
        header_row = row
        ...
        for task in tasks_data:
            ...
            row_data = [
                task['project_name'], task['customer'], task['product_code'],
                task['category'], task['task_name'], task['owner_name'],
                due_str, task['days_late'], task['status']
            ]

            fill = light_red if task['days_late'] > 14 else light_orange
        ...
        file_path = os.path.join(temp_dir, f"NPI_Overdue_Tasks_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx")
        wb.save(file_path)
        logger.info(f"Excel task scaduti salvato: {file_path}")
        return file_path
    ...
    def export_npi_overview_report(self, year_filter=None, client_filter=None, project_ids=None):
        """
        ...
        # Filtra per project_ids se specificato
        if project_ids:
            report_data['projects'] = [
                p for p in report_data['projects'] if p['project_id'] in project_ids
            ]
            if not report_data['projects']:
                raise ValueError("Nessun progetto da esportare con i filtri selezionati.")
