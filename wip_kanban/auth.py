# -*- coding: utf-8 -*-
"""
auth.py — Validazione token monouso e gestione sessione per il server Kanban.
"""
import functools
import logging
...
SESSION_USER_KEY = "wip_kanban_user"

# Permessi accettati nei token emessi da DocumentManagement:
# - 'crea_kanBan_produzione' → pagina Carica (sotto _execute_authorized_action)
# - 'wip_kanban_pick'        → pagina Preleva (sotto _execute_simple_login)
ALLOWED_PERMISSIONS = {"crea_kanBan_produzione", "wip_kanban_pick"}


def validate_token(token: str, expected_page: str):
    """Valida un token monouso su WipKanbanWebSessions e restituisce i dati utente."""
    conn = db.get_conn()
    try:
        cur = conn.cursor()
        cur.execute(
            """SELECT UserId, UserName, Permission, Page, UsedAt, ExpiresAt
               FROM Traceability_RS.ind.WipKanbanWebSessions
               WHERE Token = ?""",
            (token,),
        )
        row = cur.fetchone()
        ...
        cur.execute(
            "UPDATE Traceability_RS.ind.WipKanbanWebSessions SET UsedAt = GETDATE() WHERE Token = ?",
            (token,),
        )
        conn.commit()
